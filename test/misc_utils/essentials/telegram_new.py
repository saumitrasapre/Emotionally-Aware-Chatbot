...
class TelegramInput(InputChannel):
    ...
    def get_metadata(self, request: Request) -> Optional[Dict[Text, Any]]:
        ...
        if "message" in request.json:
            msg_sender = request.json.get("message").get("from") ## contains id,is_bot,first_name,last_name,username,language_code
        elif "callback_query" in request.json:
            msg_sender = request.json.get("callback_query").get("from") ## contains id,is_bot,first_name,last_name,username,language_code
        # msg_sender_2 = request.json.get("message").get("chat") ## contains id,first_name,last_name,username,type,default_quote,photo,pinned_message,permissions
        return msg_sender
